...
int main(int argc, char **av) {
  ...
  for (size_t i = 0; i < 1; i++) {
    MMT mmt(&g, /*L*/ 10000,/*T*/ 45, /*time limit*/ 600, /*pool size*/ 10, true); // MMTGraph * graph, int L, int T, int time_limit_sec, int pool_size = 99, double pGreedy = 0.5
  ...
  for (int i = 0; i < v.size(); i++) {
    std::cout << v[i][v[i].size()-1].k << ',' << v[i][v[i].size()-1].timeStampEnd << ' ' << v_totTime[i] << '\n';
